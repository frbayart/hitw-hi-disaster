...
# Create model
def multilayer_perceptron(input_tensor):
...
features_placeholder = tf.placeholder(features.dtype, features.shape, 'features_placeholder')
labels_placeholder = tf.placeholder(labels.dtype, labels.shape, 'labels_placeholder')
...
loss_op = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(
    logits=logits, labels = labels_placeholder, name='toto'), name='mean-toto')
...
### read an image
imgafter = mpimg.imread('after.png')
imgafter1 = imgafter.reshape(-1, 4)[:, 0:3]
print("imgafter1.shape: ", imgafter1.shape)
...
with tf.Session() as sess:
    ...
    with graph.as_default():
        ...
        # restored_logits = graph.get_tensor_by_name('dense/logits:0')

        # pred = graph.get_tensor_by_name('pred:0')

        y = tf.nn.softmax(logits)
        classs = y.eval({features_placeholder: imgafter1})
        print("classs: ", classs)
    sess.close()
